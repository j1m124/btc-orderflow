...
impl SymbolPickerState {
    pub fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
        let query_input = cx.new(|cx| {
            InputState::new(window, cx).placeholder("Search ticker or company\u{2026}")
        });
...
impl Render for SymbolPickerState {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let tabs = h_flex().gap_1().flex_wrap().children(
            FilterTab::all().into_iter().map(|tab| {
                let is_active = tab == active_tab;
                let id = SharedString::from(format!("picker-tab-{}", tab.label()));
                let mut btn = Button::new(id).label(tab.label()).small();
                btn = if is_active { btn.primary() } else { btn.ghost() };
                btn.on_click(cx.listener(move |this, _ev, _w, cx| {
                    this.active_tab = tab;
                    this.highlight = 0;
                    cx.notify();
                }))
        ...
        let list_element: gpui::AnyElement = if total_visible == 0 {
            let msg = if trimmed_query_empty {
                match active_tab {
                    FilterTab::All => "No symbols available",
                    FilterTab::Class(_) => "No symbols in this category yet",
                }
            } else {
                match active_tab {
                    FilterTab::All => "No symbols match",
                    FilterTab::Class(_) => "No symbols in this category yet",
                }
            };
            div()
        ...
        let card = v_flex()
            ...
            .child(div().px_4().pb_2().child(tabs))
