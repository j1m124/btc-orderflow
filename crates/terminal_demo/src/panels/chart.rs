...
impl ChartState {
    ...
    pub fn new(
        ...
        // Locked design choice: every fresh chart is born with a Volume
        // overlay AND a Session VWAP overlay (intraday-only — Session VWAP
        // self-disables on 1d via the chart paint-side guard). `switch_*`
        ...
        state.add_indicator(Box::new(crate::indicators::SessionVwapParams::default()));
        state
    }
...
pub fn render(
    ...
    // Session-toggle dropdown (RTH vs Ext). Same focus-scoping so the action
    // dispatches up through this chart panel.
    let session_focus = focus.clone();
    let session_btn = Button::new("chart-session-select")
        .label(SharedString::from(state.session().label()))
        .small()
        .ghost()
        .dropdown_menu(move |menu, _, _| {
            let mut menu = menu.action_context(session_focus.clone());
            for s in Session::ALL {
                menu = menu.menu(
                    SharedString::from(s.label()),
                    Box::new(ChangeChartSession(SharedString::from(s.as_str()))),
                );
            }
            menu
        });
