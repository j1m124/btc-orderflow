...
pub struct TabPanel {
    ...
    /// While true, the panel-local split preview is suppressed because the
    /// cursor is in the dock area's window-edge drop strip and the strip's
    /// preview takes precedence.
    suppress_split_preview: bool,
    /// Pinned TabPanels can't be dragged, dropped into, or closed via the tab
    /// strip. Used by host apps to mark certain TabPanels (e.g. singleton
    /// toolbar-managed panels) as non-rearrangeable.
    pinned: bool,
...
impl Panel for TabPanel {
    ...
    fn closable(&self, cx: &App) -> bool {
        ...
        // Pinned TabPanels never show a tab close button — they're managed
        // entirely by the host app (e.g. via toolbar toggles).
        if self.pinned {
            return false;
        }
...
impl TabPanel {
    ...
    /// Index of the panel within this tab panel, or `None` if not present.
    pub fn index_of_panel(&self, panel: &Arc<dyn PanelView>) -> Option<usize> {
        self.panels.iter().position(|p| p == panel)
    }

    /// Pin this TabPanel so its tabs can't be dragged out, dropped into, or
    /// closed via the tab strip. Useful for host-managed singleton panels.
    pub fn set_pinned(&mut self, pinned: bool, _: &mut Window, cx: &mut Context<Self>) {
    ...
    fn draggable(&self, cx: &App) -> bool {
        !self.pinned && !self.is_locked(cx) && !self.is_last_panel(cx)
    }
    ...
    fn render_active_panel(
        ...
        v_flex()
            ...
            .when(state.droppable, |this| {
                let suppress = self.suppress_split_preview;
                this.on_drag_move(cx.listener(Self::on_panel_drag_move))
                    .child(
                        div()
                            ...
                            // Only auto-show the local preview when the cursor
                            // isn't currently inside the dock area's window-edge
                            // strip — otherwise the strip's preview shows alone.
                            .when(!suppress, |this| {
                                this.group_drag_over::<DragPanel>("", |this| this.visible())
                            })
                            .on_drop(cx.listener(|this, drag: &DragPanel, window, cx| {
                                this.suppress_split_preview = false;
    ...
    fn on_panel_drag_move(
        ...
        // If the cursor is inside the dock area's window-edge drop strip, the strip
        // takes precedence — suppress this panel's local split preview so we don't
        // show two highlights at once.
        let in_edge_zone = if let Some(dock) = self.dock_area.upgrade() {
            let dock_bounds = dock.read(cx).bounds;
            let zone = px(super::WINDOW_EDGE_DROP_ZONE);
            position.x < dock_bounds.left() + zone
                || position.x > dock_bounds.right() - zone
                || position.y < dock_bounds.top() + zone
                || position.y > dock_bounds.bottom() - zone
        } else {
            false
        };

        if in_edge_zone != self.suppress_split_preview {
            self.suppress_split_preview = in_edge_zone;
            cx.notify();
        }
        if in_edge_zone {
