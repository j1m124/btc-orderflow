...
**Subsystems.** Indicators (`indicators/`): trait-object plugins (`IndicatorKind::compute(&[Candle], ComputeCtx) → IndicatorOutput`) with overlay-vs-pane placement; `ComputeCtx` carries footprint / liquidation / open-interest / mark-price / book-imbalance data so indicators don't own subscriptions. Two derived orderflow indicators reuse existing wire data with **no protocol/server change**: `net_ls` (net long/short flow = `sign(delta) × |ΔOI|` from taker delta + OI) and `ob_imbalance` (book bid/ask skew, normalized [−1,+1], at a depth-% band) — each is both a pane (signed histogram) and a `bar_stat` row (net L/S as one row; OB imbalance as one row per checked depth preset, `indicators/ob_imbalance.rs::OB_DEPTHS_PCT`). OB imbalance reads the heatmap's book time-series: the deep book is reduced once per snapshot to a compact `BookImbalanceSample` in `panels.rs` (`maybe_refresh_book_imbalance`, ~1s throttle) and the old `refresh_chart_heatmap_sub` is generalized to `refresh_chart_book_sub` (gated on a heatmap instance **or** any OB consumer). The **orderbook heatmap is itself an indicator** (`indicators/ob_heatmap.rs`, `OrderbookHeatmapParams`) — a *façade*: a singleton overlay kind added from the "+ Indicator" picker / managed via its chip, but its `compute` returns the no-op `IndicatorOutput::Heatmap` marker; the real render stays the GPU-texture blit behind candles (`paint/heatmap.rs::HeatmapLayer`), which `ChartState` feeds from the instance's params each frame (the layer's `enabled`/`settings` are mirror fields synced in `refresh_heatmap`). The book sub stays up while the instance exists (hiding is paint-only; × removes + tears down). Its dual-handle log colour slider can't be a declarative form, so it returns a bespoke view via the new `IndicatorKind::custom_settings_view` hook that `indicator_settings.rs` hosts (the one kind using it). Drawings (`drawings/`): tool state + serde shapes anchored in (time, price) space + a persisting service. Volume profile (`volume_profile/`): shared compute/paint for the VRVP indicator and FRVP drawing. Settings (`settings_form/`): declarative form framework used by every settings surface; edits route through typed targets, never mutate directly. Screenshot (`screenshot.rs`): captures gpui's canvas via `toBlob` before opening the preview dialog.
