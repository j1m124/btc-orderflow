...
# Source .env if present so build-time `option_env!` calls (CENTOFLOW_BASE_URL
# etc.) see the values. `set -a` auto-exports every var defined inside the
# sourced file, then `set +a` restores normal scoping. Silently no-op when
# .env is absent — CI uses workflow env vars instead.
