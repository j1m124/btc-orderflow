...
/// Global kill-switch for dialog open animations. When `false`, every
/// dialog opens instantly regardless of its per-instance `animated` flag —
/// useful when the host app exposes a user preference for "reduce motion".
/// Defaults to `true` so existing behaviour is unchanged.
static ANIMATIONS_ENABLED: AtomicBool = AtomicBool::new(true);

/// Toggle the global dialog-animation kill-switch. Affects every subsequent
/// dialog open; in-flight animations are not interrupted.
pub fn set_animations_enabled(enabled: bool) {
    ANIMATIONS_ENABLED.store(enabled, Ordering::Relaxed);
}

/// Current state of the global dialog-animation kill-switch.
...
pub(crate) struct DialogProps {
    ...
    keyboard: bool,
    /// Play the open slide-down + fade-in animation. Default `true` to
    /// preserve existing behaviour; flip off via `Dialog::no_animation`
    /// for dialogs that should appear instantly.
...
impl Dialog {
    ...
    /// Disable the open slide-down + fade-in animation so the dialog
    /// appears instantly. Default behaviour (animated) is preserved when
    /// this is not called.
    pub fn no_animation(mut self) -> Self {
        self.props.animated = false;
        self
    }
...
impl RenderOnce for Dialog {
    fn render(mut self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        // `no_animation()` collapses the open animation to a near-zero
        // tween — visually instant on the first frame. CANNOT be exactly
        // zero: gpui's animation computes `delta = elapsed / duration`,
        // which becomes NaN at zero duration and trips the internal
        // `0.0..=1.0` debug_assert. 1ms is plenty short to look instant
        // and large enough to make the first frame's delta clamp to 1.0.
        let animation_duration = if self.props.animated && animations_enabled() {
            *ANIMATION_DURATION
        } else {
