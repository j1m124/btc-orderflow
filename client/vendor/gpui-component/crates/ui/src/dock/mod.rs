...
use crate::{ActiveTheme, ElementExt, Placement, geometry::AxisExt};

/// Width of the window-edge docking strip (in px). When a tab is dragged within
/// this distance of the dock area's outer edge, the drop is treated as a
/// whole-window split rather than a panel-local split.
pub(crate) const WINDOW_EDGE_DROP_ZONE: f32 = 28.0;
...
impl DockArea {
    ...
    /// Dock the given panel as a top-level row/column at the specified window edge.
    ///
    /// Unlike a panel-local edge drop (which splits whichever TabPanel was hit), this
    /// always inserts the panel as a full-width or full-height band along the dock
    /// area's outer edge. If the current center is already a Split whose axis matches
    /// the placement, the new panel is inserted at the head/tail of that stack;
    /// otherwise the existing center is wrapped in a fresh top-level Split.
    ///
    /// `size` is the cross-axis size of the new band (width when docked Left/Right,
    /// height when docked Top/Bottom). `None` falls back to the average of existing
    /// children, which is roughly half the dock for a single-pane center.
    pub fn dock_to_window_edge(
        &mut self,
        panel: Arc<dyn PanelView>,
        placement: Placement,
        size: Option<Pixels>,
        ...
        // Wrap the dropped panel in its own TabPanel so it shows a tab strip.
        let new_tab_entity = cx.new(|cx| {
            let mut tp = TabPanel::new(None, weak_self.clone(), window, cx);
            tp.add_panel(panel, window, cx);
            tp
        });
        let new_tab_arc: Arc<dyn PanelView> = Arc::new(new_tab_entity.clone());

        // If center is a Split with a matching axis, just insert into the existing stack.
        // The DockItem.items field is decorative for serialization (dump walks the view), so
        // we only need to update the StackPanel view.
        if let DockItem::Split { axis, view, .. } = &self.center {
            if axis.is_horizontal() == placement.is_horizontal() {
                let stack = view.clone();
                let weak_dock = weak_self.clone();
                stack.update(cx, |stack, cx| {
                    let len = stack.panels_len();
                    match placement {
        ...
        // Otherwise: wrap the existing center in a new top-level Split with the dropped
        // panel placed at the appropriate end. When a `size` is given, only the new
        // band is sized — the existing center keeps an auto (None) size so it absorbs
        // the rest of the dock area.
        let new_tab_item = DockItem::Tabs {
            size: None,
            items: vec![],
            active_ix: 0,
            view: new_tab_entity.clone(),
        };

        let existing_center = self.center.clone();
        let (items, sizes) = match placement {
            Placement::Left | Placement::Top => (
                vec![new_tab_item, existing_center],
                vec![size, None],
            ),
            Placement::Right | Placement::Bottom => (
                vec![existing_center, new_tab_item],
                vec![None, size],
            ),
        };
        ...
        self.set_center(new_center, window, cx);
        cx.emit(DockEvent::LayoutChanged);
    }
...
impl Render for DockArea {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        // Window-edge drop zones are active when the dock area isn't zoomed/locked and
        // the center isn't a free-form Tiles layout (Tiles has its own drag semantics).
        let edge_zones_enabled = self.zoom_view.is_none()
            && !self.locked
            && !matches!(self.center, DockItem::Tiles { .. });
        ...
        div()
            ...
            .when(edge_zones_enabled, |this| {
                this.child(self.render_window_edge_zone(Placement::Left, cx))
                    .child(self.render_window_edge_zone(Placement::Right, cx))
                    .child(self.render_window_edge_zone(Placement::Top, cx))
                    .child(self.render_window_edge_zone(Placement::Bottom, cx))
            })
    }
}

impl DockArea {
    /// Render one of the four edge drop zones. The strip stays invisible until a
    /// `DragPanel` is dragged over it; on drop, the panel is detached from its
    /// source TabPanel and docked at the corresponding window edge.
    fn render_window_edge_zone(
        &self,
        placement: Placement,
        cx: &mut Context<Self>,
    ) -> impl IntoElement {
        let zone = px(WINDOW_EDGE_DROP_ZONE);
        let drop_color = cx.theme().drop_target;

        let id = match placement {
            Placement::Left => "dock-edge-left",
            Placement::Right => "dock-edge-right",
            Placement::Top => "dock-edge-top",
            Placement::Bottom => "dock-edge-bottom",
        };
        ...
        div()
            ...
            .map(|this| match placement {
                Placement::Left => this.left_0().top_0().bottom_0().w(zone),
                Placement::Right => this.right_0().top_0().bottom_0().w(zone),
                Placement::Top => this.top_0().left_0().right_0().h(zone),
                Placement::Bottom => this.bottom_0().left_0().right_0().h(zone),
            })
            .drag_over::<tab_panel::DragPanel>(|this, _, _, _| this.visible())
            .on_drop(cx.listener(
                move |this: &mut DockArea, drag: &tab_panel::DragPanel, window, cx| {
                    let panel = drag.panel.clone();
                    let source = drag.tab_panel.clone();
                    source.update(cx, |tp, cx| {
                        tp.detach_panel(panel.clone(), window, cx);
                        tp.remove_self_if_empty(window, cx);
                    });
                    this.dock_to_window_edge(panel, placement, None, window, cx);
                },
            ))
    }
}
